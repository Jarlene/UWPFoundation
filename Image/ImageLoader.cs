...
namespace Image
{
    public class ImageLoader
    {
            ...
            CancellationTokenSource cancellationTokenSource)
        {
            ...
            try
            {
                LogUtils.Log("[network] loading " + imageUrl);
                ...
                if (randStream == null)
                {
                    LogUtils.Log("[error] failed to download: " + imageUrl);
                    return null;
                }
                ...
                LogUtils.Log("[network] loaded " + imageUrl);
                if (_ImageConfig.CacheMode != CacheMode.NoCache)
                {
                    ...
                    _ImageConfig.CacheMode == CacheMode.OnlyStorageCache)
                    {
                        ...
                        if (imageUri.IsWebScheme())
                        {
                            ...
                            {
                                LogUtils.Log(string.Format("{0} in task t-{1}", imageUri, Task.CurrentId));
                                // Async saving to the storage cache without await
                                var saveAsync = _ImageConfig.StorageCacheImpl.SaveAsync(imageUrl, randStream)
                                      .ContinueWith(task =>
                                      {
                                          LogUtils.Log(string.Format("{0} in task t1-{1}", imageUri, Task.CurrentId));
                                          if (task.IsFaulted || !task.Result)
                                          {
                                              LogUtils.Log("[error] failed to save in storage: " + imageUri);
                                          }
            ...
            {
                LogUtils.Log("[error] failed to save loaded image: " + imageUrl+ " then exception: " + ex.Message);
            }
            ...
            LogUtils.Log("[error] failed to load image stream from cache and network: " + imageUrl);
            return null;
        }

...
namespace AnyView
{
    ...
    public abstract class BaseSwipeAnimator : ISwipeAnimator
    {
        ...
        protected void DisplayAnimation(SwipeConfig config, string itemProperty, double itemTo, string clipProperty, double clipTo, Action complete)
        {
            ...
            animStory.Children.Add(AnyViewUtils.CreateDoubleAnimation(config.MainTransform, itemProperty, config.EasingFunc, itemTo, config.Duration));
            animStory.Children.Add(AnyViewUtils.CreateDoubleAnimation(config.SwipeClipTransform, clipProperty, config.EasingFunc, clipTo, config.Duration));
